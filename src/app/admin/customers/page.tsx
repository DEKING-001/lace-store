...
import { Users, Mail, Phone, MapPin, ShoppingCart } from "lucide-react";
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((customer) => (
            <div
              ...
              className="bg-white p-5 rounded-2xl border border-border hover:shadow-md transition-shadow"
            >
              <div className="flex items-start justify-between mb-3">
                <div>
                  <h3 className="font-bold text-foreground">{customer.name}</h3>
                  <p className="text-xs text-foreground/50">
                    Last order: {new Date(customer.last_order).toLocaleDateString("en-NG")}
                  </p>
                </div>
